...
;(function ($, window, document, undefined) {
    ...
    */
    ...
    Tree.prototype._renderNode = function (node, previousNode) {
        ...
        node.$el.attr('data', JSON.stringify(obj));
        node.$el.addClass('data-id-' + node._id);
        if (node.code == 'myFolder') {
            node.$el.addClass('context').addClass('myFolder');
        }
        if ((node.nodeId + "").indexOf("0.2") == 0) {
            node.$el.addClass('context');
        }
        node.$el.attr('title', node.name);
        ...
        if (this._options.wrapNodeText) {
            var wrapper = this._template.name.clone();
            node.$el.append(wrapper);
            wrapper.append(node.name);
        } else {
            node.$el.append(node.name);
        }
    ...
    Tree.prototype.search = function (pattern, options) {
        ...
        if (pattern && pattern.length > 0) {
            ...
            results = this._findNodes(pattern, 'name', modifier);
        }
